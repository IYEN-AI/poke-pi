...
}).refine((rule) => rule.action !== undefined || rule.preferMapDirection === true, {
  message: "rule must provide action or preferMapDirection"
});
...
export const GeneratedPolicySchema = z.strictObject({
  ...
  tuning: GeneratedPolicyTuningSchema.default({ avoidRecentDirections: true, preferDifferentBlock: true, interactionCandidatePatience: 2, fallbackToBaseHeuristic: true }),
